...
Colombia has an synchronic invoicing system, which means that documents will be checked immediately as you
issue them.
...
## 3) Approved by tax authority (DIAN)
```json
{
    "header": {
        "document_status" : 1,
...
## 3B) Rejected by tax authority (DIAN)
```json
{
    "header": {
        "document_status" : 1,
